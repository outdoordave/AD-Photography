...
export function buildLatest(opts: { stories: Doc[]; trips: Doc[]; albums: Doc[]; lang: Lang; showStories: boolean; prefix: string }): LatestCard[] {
  ...
  if (showStories) {
    for (const s of stories) {
      ...
      rows.push({ date: d.date || '', card: { tag: TAG.story[lang], title: title || '', meta: [monthYear(d.date, lang), cat].filter(Boolean).join(' · '), href: `${prefix}/stories/${s.slug}`, image: d.cover || '', c1: pal.c1, c2: pal.c2, img: pal.img } });
    }
  }
