...
- **Build: fertige Bilder überspringen** (`d219148`): Entscheidender Befund — **jeder CMS-Upload ist bereits fertig optimiert**. `toOptimized` wandelt im Browser nach WebP q85 / max. 2400 px um und lädt **nur diese Datei** hoch (das JPEG bleibt auf Davids Rechner). Der Build rechnete sie trotzdem jedes Mal erneut durch und drückte dabei q85 auf q80 — Zeit **und** Qualität verschenkt. Jetzt werden WebP ≤ 2400 px unverändert durchgereicht; nur Alt-Bestand (Roh-JPG/PNG) wird noch verarbeitet — eine feste, **nicht wachsende** Menge. Damit bleibt die Bild-Phase konstant, statt linear mit der Bibliothek zu wachsen. Gemessen: 51 von 64 übersprungen; kalt 20,8 s statt zuvor 64 s auf Cloudflare; Push → live **3:35** (vorher allein der Build 4:25). Qualität verifiziert **bitgleich** zur Repo-Fassung (die volle Fassung ist dadurch etwas größer: 746 → 859 KB, wird dank srcset aber nur noch auf großen Retina-Schirmen geladen).
