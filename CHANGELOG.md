...
- **Build-Cache** (`69110c0`): `optimize-uploads` komprimiert bei **jedem** Build alle 48 MB neu. Neu: das fertige Ergebnis wird unter einem Hash aus (Dateiinhalt + Parametern) in `node_modules/.cache/` abgelegt und byte-identisch zurückgeschrieben. **Lokal gemessen: 12,3 s → 0,106 s**, Ergebnis per Prüfsumme als **bitgleich** verifiziert.
  - ⚠️ **KORREKTUR (Build-Log vom 23.09. geprüft): auf Cloudflare wirkt dieser Cache NICHT** — dort läuft `npm clean-install`, das `node_modules` (und damit den Cache) vor jedem Build löscht. Das Log zeigt „0 aus Cache, 64 neu berechnet". Der Cache nützt nur lokal bzw. in CI-Umgebungen, die `node_modules` behalten. Die Bild-Phase bleibt auf Cloudflare bei **64 s = 24 %** des 265-s-Builds. Offene Idee: Varianten ins Repo committen (+16 MB) und im Build überspringen, wenn bereits vorhanden.
...
- **Build: fertige Bilder überspringen** (`d219148`): Entscheidender Befund — **jeder CMS-Upload ist bereits fertig optimiert**. `toOptimized` wandelt im Browser nach WebP q85 / max. 2400 px um und lädt **nur diese Datei** hoch (das JPEG bleibt auf Davids Rechner). Der Build rechnete sie trotzdem jedes Mal erneut durch und drückte dabei q85 auf q80 — Zeit **und** Qualität verschenkt. Jetzt werden WebP ≤ 2400 px unverändert durchgereicht; nur Alt-Bestand (Roh-JPG/PNG) wird noch verarbeitet — eine feste, **nicht wachsende** Menge. Damit bleibt die Bild-Phase konstant, statt linear mit der Bibliothek zu wachsen. **Am echten Cloudflare-Build gemessen** (Logs 18:09 vs. 18:37): 51 von 64 Bildern übersprungen, Bild-Phase **64 s → 46 s**, Gesamt-Build **265 s → 222 s (4,4 → 3,7 min)**. Qualität verifiziert **bitgleich** zur Repo-Fassung (die volle Fassung ist dadurch etwas größer: 746 → 859 KB, wird dank srcset aber nur noch auf großen Retina-Schirmen geladen).
- **Entscheidung (David, 23.09.): Teil B wird NICHT umgesetzt.** Die verbleibenden ~46 s der Bild-Phase sind fast vollständig die 254 srcset-Varianten, die bei jedem Build neu entstehen (skaliert mit der Bibliothek: bei 500 Fotos ~5 min). Sie ließen sich durch Mitcommitten der Varianten eliminieren (Build ~3,0 min), das würde das Repo aber um 16 MB vergrößern und **jedes neue Foto mit 5 statt 1 Datei** ablegen — bei einer Fotografie-Seite langfristig der schlechtere Tausch. Bewusst vertagt: bei deutlich größerer Bibliothek lieber gleich ein Bild-CDN (Cloudflare Images) statt Varianten im Repo.
- **Nachgezogen** (`a68dad0`): Die Live-Messung nach dem ersten Deploy zeigte, dass nur 4 von 14 Bildern `srcset` hatten — die **schwersten** Bilder (die fünf Hero-Dias der Startseite, Duo-Kacheln, Stories-Hero) und das Logo selbst hingen an noch nicht verdrahteten Stellen. Nachgeholt; zusätzlich eine **320px-Stufe** ergänzt (Nav-Logo wird nur 93 px breit angezeigt → lädt jetzt **16 KB statt 137 KB**).
- **Gemessen live (Startseite, Handy 375 px @ DPR 2): 3.595 KB → 770 KB Bilddaten = −79 %.** Am Desktop mit Retina fällt die Ersparnis kleiner aus, weil ein formatfüllender Hero dort echte ~2000 px braucht — dort wirken vor allem Logo und Kacheln. Alle 14 Bilder der Startseite haben jetzt `srcset`.
...
- Verifiziert: esbuild grün, Lib gegen echtes Manifest getestet (39 Bilder, Ordner Alben/Firsts/USA-2023, Suche), Story-Seite baut 200 ohne Fehler. **Kein Schema-Eingriff → kein Re-Index.** Interaktiver Test im Cloud-Admin durch David.
...
- **Reise-Detail v2 Feinschliff:** Karten-Breiten-Zittern (`52211c9`, Grid `minmax(0,fr)` — Caption zog Spalte per min-content), Caption-Quetschung „S…" (`192e42b`, Dots umbrechen), Karten-Höhe fix/Fahrzeug-Pille gold/Off-by-one (`71b153d`).
- **Offen:** Startseiten-Sektionsköpfe 01–04 sind Design-Labels (kein CMS-Inhalt); Reise-Detail-v2-Hero (Titel/Meta) editiert sich über die Reise in der Sidebar (Stationen sind live).
...
- **11 Repo-JPEGs zu WebP migriert (inkl. EXIF):** Einmal-Skript `scripts/migrate-jpeg-to-webp.mjs` (Q85, Breite ≤2400, GPS-sicherer EXIF-Re-Mux) + 21 Referenzen in 6 Inhalten von `.jpg/.jpeg` → `.webp` (inkl. Crop-Objekte). Originale bleiben liegen (Sicherheitsnetz). Manifest/Meta neu (39 Bilder). 0 kaputte Referenzen. Commit: `153c073`.
- Dateien: web/scripts/optimize-uploads.mjs, web/scripts/migrate-jpeg-to-webp.mjs, web/scripts/lib/exifCamera.mjs, web/src/lib/mediaCloud.ts, web/src/styles/global.css, web/src/data/* + web/src/content/*, web/public/uploads/*.webp (+ IMG_6654-2.webp), uploads-manifest/meta.
